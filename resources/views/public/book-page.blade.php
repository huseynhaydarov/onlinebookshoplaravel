                    ...
                    <div class="content-area">
                        @if($term = request('term'))
                            <div class="alert alert-info my-3">
                                Результат поиска для <strong>{{$term}}</strong>
                            </div>
                        @endif
                        @if(! $books->count())
                            <div class="alert alert-warning my-4">Нет доступных книг</div>
                        @else
                            <div class="card my-4">
                                <div class="card-header bg-dark">
                                    <h4 class="text-white">Все книги</h4>
                                </div>
                                @if(isset($categoryName))
                                    <div class="alert alert-info m-3">
                                        Книги из категории <strong>{{$categoryName}}</strong>
                                    </div>
                                @endif
                                @if(isset($authorName))
                                    <div class="alert alert-info m-3">
