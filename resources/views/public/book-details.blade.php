                    ...
                    <div class="content-area">
                        <div class="card my-4">
                            <div class="card-header bg-dark">
                                <h4 class="text-white">Детали книги</h4>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="author mb-2">
                                            Автор: <a href="{{route('author', $book->author->slug)}}">{{$book->author->name}}</a>
                                        </div>
                                        @if(($book->quantity) > 1)
                                            <div class="badge badge-success mb-2">В наличии</div>
                                        @else
                                            <div class="badge badge-danger mb-2">Нет в наличии</div>
                                        @endif
                                        @if($book->discount_rate)
                                            <h6><span class="badge badge-warning">{{$book->discount_rate}}% скидка</span></h6>
                                        @endif
                                        <div class="book-price mb-2">
                                            <span class="mr-1">Цена</span>
                                            @if($book->discount_rate)
                                                <span></span><strong class="line-through">{{$book->init_price}}c</strong>
                                            @endif
                                                <span>сейчас</span><strong> {{$book->price}}c</strong>
                                            @if($book->discount_rate)
                                                <div><strong class="text-danger">Экономия {{$book->init_price - $book->price}}c</strong></div>
                                            @endif
                                        </div>
                                        <div class="book-category mb-2 py-1 d-flex flex-row border-top border-bottom">
                                            ...
                                            <a href="#review-section" class="mr-4"><i class="fas fa-comments"></i> Отзывы</a>
                                        </div>
                                        ...
                                        <form action="{{route('cart.add')}}" method="post">
                                            ...
                                            <span class="quantity-input mr-2 mb-2">
                                                ...
                                                <input title="КОЛ-ВО" name="quantity" type="text" value="1" class="qty-text">
                                                <a href="#" class="cart-plus" id="cart-plus">+</a>
                                            </span>
                                                <input type="hidden" name="book_id" value="{{$book->id}}">

                                                <button type="submit" class="btn btn-danger btn-sm"><i class="fas fa-shopping-cart"></i> Добавить в корзину</button>
                                            </div>
                                        </form>
